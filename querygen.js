...
if(query.charat(query.length-1) == ';')
     query = query.substr(0, query.length-1);//trimming the semicolon off the end before appending the statement
if(query.contains("GROUP BY")==true){
		queryEnd = query.substr(query.indexof("GROUP BY"), query.length - query.indexof("GROUP BY"));
...
//con_hours is queried
	...
	retQuery = "SELECT * FROM con_hours";
...
//this returns the data in blocks of <interval> minutes at a time. Intervals will always begin on the hour.
	retQuery = "SELECT DATE_SUB(DATE_SUB(polltime, INTERVAL MOD(MINUTE(polltime), " + interval;
